// use-mydb.ts — 私有存储域状态机（0.10.18 自 views/OpsMydb.vue 迁入）。
//
...
import { getTables, readData, writeData, queryStockdb } from '../api/data'
...
function unionKeys(items: Record<string, unknown>[]): string[] {
...
function payloadToRows(data: unknown): { columns: string[]; rows: string[][] } {
  ...
  if (Array.isArray(data)) {
    ...
    if (isPlainObject(data[0])) {
      ...
      const cols = unionKeys(objs)
      return { columns: cols, rows: objs.map((item) => cols.map((c) => displayValue(item[c]))) }
    }
  ...
  if (isPlainObject(data)) {
    ...
    if (allObj) {
      const objs = entries.map(([, v]) => v as Record<string, unknown>)
      const cols = unionKeys(objs)
      return {
        columns: ['键', ...cols],
        rows: entries.map(([k, v], i) => [k, ...cols.map((c) => displayValue(objs[i][c]))]),
      }
    }
...
function parseSingleValue(text: string): unknown {
  const t = (text || '').trim()
  if (!t) return ''
  try {
    return JSON.parse(t)
...
function parseBatchPayload(text: string): [string, unknown][] {
  const t = (text || '').trim()
  if (!t) throw new Error('请输入批量 JSON')
  const obj: unknown = JSON.parse(t)
  const items = Array.isArray(obj)
    ? obj
    : isPlainObject(obj) && Array.isArray(obj.items) ? obj.items : null
  if (!items) throw new Error('批量格式需为 [["k",值],...] 或 {"items":[["k",值],...]}')
  if (!items.length) throw new Error('批量 items 不能为空')
  return items.map((pair) => [String((pair as unknown[])[0]), (pair as unknown[])[1]])
}

// /api/data/read 两种返回形态：单条 = 有 value；整表 = 有 keys(+values)
...
export function useMydb() {
  // ---- 表清单 ----
  const tables = ref<string[]>([]) // 自定义表名数组（后端已过滤上游保留表）
  ...
  const readResult = ref<ReadResult | null>(null) // {table,key,value} 或 {table,keys,values}
  const readError = ref<string | null>(null)

  // ---- 写入表单 ----
  const writeMode = ref<'single' | 'batch'>('single') // single | batch
  ...
  const queryColumns = ref<string[]>([]) // 动态列名
  const queryRows = ref<string[][]>([]) // 原始行（数组数组），表格数据在 computed 里转换
  ...
  async function loadTables(manual = false) {
    ...
    try {
      ...
      tablesError.value = errText(e, '表清单接口不可用')
    } finally {
  ...
  async function doRead() {
    ...
    try {
      readResult.value = await readData(selTable.value, readKey.value.trim()) as ReadResult
    } catch (e) {
      readResult.value = null
      readError.value = errText(e, '读取失败')
  ...
  async function doWrite() {
    ...
    let payload: { table: string; key?: string; value?: unknown; items?: [string, unknown][] }
    try {
      ...
      ElMessage.warning(errText(e, '批量 JSON 格式有误')) // 客户端格式校验失败，还没发请求
      return
    }
    ...
    const confirmText = writeMode.value === 'batch'
      ? `确定向表「${table}」批量写入 ${payload.items?.length ?? 0} 条？写入后可用读取功能验证。`
    ...
    writing.value = true
    try {
      const r = await writeData<{ msg?: string; written?: number }>(payload as Record<string, unknown>)
      ElMessage.success(r?.msg ?? `已写入 ${r?.written ?? 0} 条`)
      await loadTables() // 表可能是新建的，刷新清单
    } catch (e) {
      ElMessage.error(errText(e, '写入失败')) // 后端 400/501/500 的 error 文案
    } finally {
      writing.value = false
    }
  }
  ...
  async function doQuery() {
    ...
    try {
      ...
      queryError.value = errText(e, '查询失败')
      queryDone.value = true
      queryColumns.value = []
      queryRows.value = []
