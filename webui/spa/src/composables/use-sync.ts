// use-sync.ts — 数据同步页业务状态机（0.10.18 自 views/OpsSync.vue 迁入）。
//
// 承接（哲学 #2 复杂度定向转移）：2 个数据源（status/history）+ 触发动作的
// 状态与全部取数/操作逻辑，无 DOM。同步日志已按第五批归属表移出本域
// （唯一的家 = 日志中心，getLog 消费方在 views/OpsLogs.vue）。
// 取数走 api/status.ts 封装（依赖方向：composable → api，渲染层不碰 fetch）。
//
// 轮询不在这里注册——由编排壳 usePolling 统一调度（可见性节拍在 use-polling）。
import { ref, computed } from 'vue'
// ElMessage / ElMessageBox 是"命令式"弹窗，不走模板组件，必须显式 import
import { ElMessage, ElMessageBox } from 'element-plus'
import { getStatus, getHistory, runSync } from '../api/status'
import { errText } from '../api/http'
import type { StatusPayload, SyncHistoryRow } from '../types/api'

export function useSync() {
  const status = ref<StatusPayload | null>(null) // /api/status 全量载荷（含 container/sync/disk/calendar...）
  const history = ref<SyncHistoryRow[]>([]) // /api/history 同步历史（后端按时间正序追加，末尾最新）
  const loading = ref(false) // 首拉/手动刷新中
  const error = ref<string | null>(null) // 最近一次失败文案（页面顶部 alert，不打断使用）
  ...
  async function loadHistory() {
    try {
      ...
      error.value = errText(e, '同步历史接口不可用')
    }
  }
  ...
  async function doSync(hot: boolean) {
    ...
    syncBusy.value = true
    try {
      const r = await runSync(hot) as { msg?: string } | null
      ...
      ElMessage.error(errText(e, '启动同步失败'))
    } finally {
