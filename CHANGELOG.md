...
## [0.11.0] — 2026-09-19（**复权因子通道接线**：v_daily 复权 5 列从设计内 NULL 变可用）

> 定位：0.11 主线——把仓库层最大的半成品做完。`adjust_provider` 注入点自 0.10.10
> 埋点以来从未绑定（组合根唯独缺它），v_daily 的 `adj_factor` 与 4 个 fq 列一直
> NULL 原价（0.10.44 实机取证：引擎 bar 无此 5 键，因子须走 mydb `复权:` 表通道）。
> 本版接线并补齐日期维度，backfill 历史日期口径同步修正。

- **实机取证定口径**：引擎 `复权:<code>:<date>` 表 = **带日期的除权事件序列**
  （600000 共 27 条，20000706→20260716，每条 {div, give, trans, mult, cum}，
  cum 单调递增=后复权累计因子，date=除权除息日该日起生效）——无需差分推导，
  `sink.write_daily` 的 `{code: 当日 cum}` 契约不动，修复全部落在任务层。
- **日期维度（修 0.10.10 口径失真隐患）**：`_build_factor_series` 按码构建事件
  序列（日期升序），沉淀日 bisect 取 ≤当日 的最后一条 cum——**backfill 历史
  日期不再拿到"今天的因子"**；缺码/未接通道 → 物化列 NULL（原价）不阻塞沉淀。
- **接线与刷新节奏**：`app._wire_warehouse_tasks` 补绑 `adjust_provider`（组合根
  唯一缺绑定的注入点）；每沉淀日全量拉取（弃用"仅周一"——周中除权滞后 ≤4 交易日），
  自然日缓存（同日补沉淀/缺口回填复用；codes 超集重拉）。通道 = 逐码读
  `复权:<code>:*`（MCP `collect_adjust_events` 新函数，TTL 300s/码，单码失败跳过），
  ~5.5k 码全量一轮分钟级，当日沉淀窗口内完成。
- **事件审计落盘**（兑现 design/warehouse.md:172 延后承诺）：warehouse.duckdb 新增
  `adjust_events` 表（PK code+date，div/give/trans/mult/cum/source_ts），首见事件
  插入（source_ts=发现时间），run_sql 可查；首轮 ~14 万行批量导入秒级。
- **历史策略**（沿用 0.10.44 先例）：接线日起沉淀的行带因子，已沉淀历史行保持
  NULL 不重写；确需补历史走 `backfill_daily_direct.py` 重灌（另行立项）。
- **测试 +6（test_warehouse 81 / 全量 513 全绿）**：bisect 按日取值、坏事件跳过、
  主链路（当日最新因子 + backfill 历史因子 1.5≠2.5 口径验证）、审计去重、
  provider 失败降级 NULL、同日缓存复用。
- **周月聚合自动受益**：`_kline_aggregate_sql` 直接读 daily 分区的 adj_factor——
  接线日后沉淀的日K，其周K/月K 物化 fq 列自动带上（历史周月不重算，幂等边界内
  新聚合周/月生效）。


## [0.10.45] — 2026-09-19（**仓库缺口自愈守卫**：水位线越过的中间日不再成永久洞）

> 起因：09-19 周六 SSH 巡检发现 **20260915（周二，交易日）整日缺失于研究仓库**
> ——`v_daily` 从 09-14（5173 行）直接跳到 09-16（5171 行），打板日检 0915 只有
> 早上 collect、无 close/沉淀记录，**W38 周K 被完整性校验卡住 4 天**（watermark:week
> 停在 0911），面板却全绿。人工已用 backfill 模式补齐（5171 行对账零差异，周聚合
> 自动级联）；本版把这条人工路径变成自动守卫。

- **根因链（实机取证）**：09-15 上游镜像故障 → 当晚 6 轮 stale-retry 全部停在
  `data_latest=20260807`，沉淀就绪门 20:00 未过（error 告警收口）→ 09-16 16:22
  追平后，调度沉淀 `days=1` 只写最新日 0916，**水位线越过 0915**。晚到钩子
  `maybe_catchup_sediment` 同为 days=1 且仅在 `watermark < latest` 时触发——
  对水位线之下的中间日空洞是盲区（与 0908 误标 `empty:` 同族，本次连标记都没有）。
- **修法：`services/warehouse_tasks.maybe_backfill_gaps()` 缺口守卫**——调度沉淀
  成功收尾后调用，自水位线向下回看**有界窗口（5 个交易日）**，发现"无分区且无
  `empty:` 标记"的洞：
  - **引擎侧探测先行**（快照 `limit=0` 与沉淀通道同源）：有 TRADED 点才回填；
    无数据则投 warn 告警待次日再探，**绝不写 `empty:` 标记**（0908 教训——误标
    会挡住追平后的回填）；
  - 回填**同步执行** `warehouse_run(days=len(可填洞), backfill=True)`：缺口感知
    扫描自水位线向下在最新洞处停住（单洞实测 ~2s），落盘即带对账，水位线只前进；
  - **告警是条件式投影**（沿用 0.10.36 哲学）：洞存在才挂（source=仓库，前缀
    "缺口自愈："），全部落盘对账后 `resolve` 撤回——不留陈旧红点；
  - **窗口必须有界是硬约束**：backfill 的 days 语义在"洞数 < days"时会扫到
    `_BACKFILL_FLOOR`（2000 年），fnOS 慢卷上是分钟级 stat 风暴——守卫自己只做
    窗口内 `.exists()`/标记查询（≤15 次），确认洞存在才触发，绝不无脑每日全史扫描；
  - `empty:` 误标的清理仍归人工（需判断力），守卫只认"无标记、无分区、窗口内
    交易日"的洞。
- **测试 +6（test_warehouse 75 全绿，全量 507 全绿）**：主链路（0915 复刻——
  洞在水位线下、探测有数据、回填对账、水位线不回退、撤警）；无洞零引擎往返；
  引擎缺数据不回填不写标记+同日告警去重+追平后自动回填；窗口有界（探测次数 =
  洞数）；标记日与非交易日不计洞。
- **顺带修复（同批，预存在缺陷）**：① 4 个时间线用例**日期腐化**（固定播种
  0904/0907/0908/0909/0911 随真实时间滑出 7 交易日窗口，09-19 起恒红）——
  `load_timeline` 抽出可 patch 时钟缝 `app._timeline_today()`（与 `_monotonic`
  同惯例），两类用例冻结窗口起点；② 巡检排障实锤运维坑：**`.env` CRLF 使 shell
  侧 token 带 `
`，放进请求头后裸 `
` 被当行结束 → Content-Length 头丢失 →
  webui 全部 POST body 静默变空**（`backfill:true` 被吞成 False，三次"成功"回填
  实际零目标）——读 token 后 `tr -d "
"`，判别法与预防已记 development-guide §5，
  NAS `.env` 已转 LF。


## [0.10.44] — 2026-09-14（**快照通道字段投影补全**：pct_chg/amplitude 不再恒 NULL）

> 起因：巡检飞牛实例时顺手核数据质量，发现 `v_daily` 的 `pct_chg`（涨跌幅）与
> `amplitude`（振幅）**整列恒 NULL**（09-08~09-14 实测 5173 行/日全空），而同两列在
> 周K/月K 里正常有值。**结论：不是数据没有，是被我们的适配层丢掉了。**

- **根因（实机取证，非推断）**：引擎单日 bar 实测带 **21 键**——除 OHLC/昨收/量额外，
  **还带 turnover / pct_chg / amplitude / vol_ratio / pb / pe_ttm / 股本市值 4 键**；
  而快照通道的适配函数 `_point_snapshot_item` 是**手写字段清单**，只挑走 11 键 →
  这 10 列在进入 sink 之前就被丢弃，sink 把 26 列建好却永远写 NULL。
  取证口径：`日k:600000:20260914`（`cmd=get`）+ 批量 SDK 双通道各测一遍，
  **17 个数值字段 20/20、7/7 全非空**；`adj_factor` 两通道都 0/20——即"引擎真没有"的
  只有复权 5 列（因子通道未接，设计内留 NULL）。
- **修法：补齐投影，保持镜像语义（不做本地重算）**——引擎值原样透传，因为
  **引擎 `pct_chg` 只保留 2 位小数**（600000 = 1.51），本地公式给 1.51187905；
  自算会让日K/周月K/引擎三处口径不一致（CHANGELOG 0.10.7「读到什么写什么」）。
  这也顺带修好 `turnover`/`vol_ratio`/`pb`/`pe_ttm`/市值股本 8 列——同一处缺陷的
  其余受害者（此前 `vol_ratio` 在 daily 也是恒 NULL，而周K 里它是"周期级无定义"）。
- **已决：不为这两列重写历史**（用户拍板）——两列可从 `(close - prev_close) /
  prev_close` 现场派生，周/月聚合本就是这个口径；为"能算出来"的列去重写 200+ 个
  不可变分区（facts 只增不改）风险收益不成比例。**新数据自动带上**（下次沉淀起），
  历史分区保持 NULL。迁移路径与"切勿盲删分区"的警告已写进设计文档 §2.5。
- **新增设计文档 §2.5「daily 镜像字段清单与可空原因台账」**：把 26 列按「引擎 bar
  原生 / 任务层注入 / 本地物化」三分类列清，并立排查纪律——**某列全 NULL 时先分类**：
  原生类为空 = bug（适配层丢字段），物化类为空 = 设计内（事件未就绪不编值）。
  两类现象相同、性质相反，别再拿「引擎没给」当万能解释。
- 测试：新增 `test_snapshot_channel_materializes_engine_mirror_fields`（快照通道
  20 字段 → 分区 26 列，断言 pct_chg/amplitude 真落进 Parquet）+
  `test_point_snapshot_passes_through_engine_mirror_fields`（MCP 面值原样透传、
  `pre_close→prev_close` 改名是唯一适配）。**CI 12 模块 512 全绿**。

### 运维留痕（同批，非代码）
- **NAS 旧镜像清理**：`stockdb-ai` 由 18 个 tag（≈4GB）清到**只留当前 0.10.43**
  （用户拍板）。compose 用精确 tag（非 `:latest`）且本地构建随时可重建 → 无启动风险；
  代价是回滚需重新 build（下次版本重建后即恢复"留一版上滚点"）。

## [0.10.43] — 2026-09-13（多市场交易日历：港股日历就绪，H1/H2 最后一项前置清掉）

> 起因：讨论「港股数据该放哪」时定下 H2 迁仓的三个前置（复权口径 / 单位 / **交易日历**）。
> 本版清掉日历这一项，并把 `app.py` 里那份与 `core/calendar_xshg.py` **重复的休市表拷贝**
> 一并消除（此前两处各存一份，存在静默漂移风险）。

- **港股日历从哪来（关键澄清）**：项目一直用 [exchange_calendars](https://github.com/gerrymanoim/exchange_calendars)
  的 XSHG 日历（`app.py` 注释与维护脚本），**该库同时收录 XHKG**——所以不需要手工编
  香港假日，也不需要换数据源：**同一个库、同一套提取口径，各提一张表**。
  已用 4.13.2 逐项核对内嵌 XSHG 表（2024/2025/2026 = 20/18/19 项**全等**），口径可信。
- **两市场必须是两张表（实证）**：2026 年内嵌表差异——
  **香港休而 A 股开 6 天**（04-03 耶稣受难节、04-07 复活节星期一翌日、05-25 佛诞翌日、
  07-01 特区成立纪念日、10-19 重阳翌日、12-25 圣诞）、
  **A 股休而香港开 11 天**（含 10-02~10-07 国庆整周）。港股无调休补假，
  故休市工作日反而比 A 股少（2026 年 14 天 vs 19 天）。
- **新增 `core/calendar_market.py`**：`MarketCalendar` 骨架 + `SH`/`HK` 冻结实例 +
  `get_calendar(market)`（未知市场**报错**，不静默回落）。模块级 `is_trading_day(...)`
  等保持 A 股默认（兼容旧调用）。**运行期仍零依赖**（纯标准库，表内嵌）——
  库只在维护期用（它依赖 pandas/numpy，不为两年一更的假日表让容器背这个包袱）。
- **港股覆盖到 2027-09-13**（XHKG 的 `last_session`），比 A 股表多 9 个月——
  顺带把「未收录年份怎么办」这个缺口推后。XSHG 库侧 2027 尚未收录
  （`last_session=2026-12-31`），故 A 股表维持原覆盖期不变。
- **消除重复拷贝**：`app.py` 内嵌的 `XSHG_HOLIDAYS` 删除，改为委托 `calendar_market.SH`
  （保留同名导出供 test/docs 引用，判定逻辑逐位一致）；`core/calendar_xshg.py` 收成
  **兼容壳**（同名再导出），interfaces/mcp 与 services 的既有调用点**零改动**。
  日志限频（未收录年份每年一次）保留在 `app.py` 外层——core 层不许加副作用。
- **台风/黑色暴雨临时休市：明确「不预判」**（写进模块 docstring 与设计文档）：
  交易所日历不收录（事后才知、无法预测）。按「数据即事实」处理——拉不到数据就跳过该日，
  **不预生成空行、不伪造闭市标记**。代价是那天成空洞（watermark 仍推进），
  由 `v_daily` 缺口巡检/补拉兜底。用例 `test_hk_typhoon_day_follows_lenient_rule`
  把这一语义锁住，防止将来有人往表里塞猜的日期。
- **维护脚本泛化**：`scripts/extract_xshg_holidays.py` → `scripts/extract_calendar_holidays.py`
  （`--calendar {XSHG,XHKG}` / `--years` / `--all`；按 `first/last_session` 夹取范围——
  越界调 `is_session` 会抛 `DateOutOfBounds`，XSHG/XHKG 的末季都是部分年份）。
- 测试：新增 `test_calendar.py` **26 例**（A 股语义不回退 / 港股圣诞与国庆差异 /
  春节区间错开 / 2027 覆盖 / 台风日仍判交易日 / 两表差异护栏 / 兼容壳与 `app` 逐位一致 /
  三种入参形态 / 未知市场报错 / **装了库则与 exchange_calendars 逐项对表**，未装则跳过）。
  **Python 全量 484 全绿**。
- 文档：`docs/design/warehouse.md` §2.2 补齐 **hk 迁仓前置条件清单**（此前只有结论
  「hk 并入 daily」而无前提）；`docs/ROADMAP.md` 主线三更新；`docker/README.md` 日历说明更新。

### 遗留事项处置记录
- ✅ **H1/H2 的日历前置**：本版清（`calendar_market.HK`，覆盖至 2027-09-13）。
- ✅ **app.py 与 core 的休市表重复拷贝**：本版消除（委托单源）。
- ⏸️ **H2 迁仓阻塞项（口径归一为不复权）**：未动——港股源目前只有前复权
  （东财 `fqt` / 腾讯 `qfq`），需先确认源能否给不复权；登记在 warehouse §2.2。
- ⏸️ **台风休市缺口巡检**：未做（接受"数据即事实" + 事后补拉；如需告警属独立需求）。
- ⏸️ H1 定时同步 / H3 一票一档：功能本体未动。
...
## [0.10.42] — 2026-09-13（打板兜底覆盖率护栏：09-09 采集异常的定性收口）

> 0.10.41 合并后继续清遗留，把 09-09 那条「竞价只采到 13 只、权威口径 66 只」
> 查到了根因并加护栏。**同一版本不二次发布**（发布纪律），故单独一版。
> 交付面只有 `services/auction_tasks.py` 的兜底护栏 + 用例，风险面最小。

- **定性（NAS 实机只读取证）**：09-09 的清单来自**兜底现算**——09-08 的 16:30 清单为空
  （`research_store.read_list("20260908")` → codes=0），于是 09-09 09:26 走
  「前一日快照兜底现算」，而那份 09-08 快照是**部分快照**（当日引擎区间语义退化，
  全市场只有 ~13 只有 bar）。**关键盲区**：`coverage.formal_usable` 当时仍为 `True`
  ——批量通道不报错，只是多数代码无 bar 被归入 `suspended`，于是没有任何告警，
  13 只清单被当成正常结果采了一天。
- **护栏 `_auction_fallback_coverage_check()`**：兜底清单 < 前一日权威 `n_samples` ×
  `_AUCTION_FALLBACK_MIN_RATIO`（0.5）→ `warning` 告警（source=打板兜底）+ 日志；
  采集记录新增 `list_source`（`stored`/`fallback`）与 `fallback_coverage`
  （`checked`/`prev_n_samples`/`ratio`/`suspicious`）。**不阻断采集**（宁可偏少也别当天
  完全没数据），也**不自动改写**清单——人工据告警处置。前日无权威值/读失败 → 保守放行不告警。
- **验证**：NAS 实机只读演练（真实 09-08 权威 88 只）——`fallback=13` → `ratio=0.1477`
  `suspicious=true` 且投递 1 条 warning；`fallback=66` → `ratio=0.75` 不告警。
  本地 4 例新用例（可疑/正常/无前值保守放行/告警通道异常不炸）。**Python 全量 458 全绿**。
- **数据面**：09-09 指标行此前已由 60 日回填以权威 kline 口径覆写（`daily.matched_count=66`），
  13 条快照作为证据保留；本版不改数据。
...
> 对账产生的「竞价/开盘口径偏差」。**数字是真的，但成因是自造的假象**——
> 告警里「竞价 75.68」实测正是当日**收盘价**（605198：K线 open=80.57/close=75.68），
> 快照 `fetched_at` 显示 21:31/21:56 采集、腾讯报文时间戳 16:14（已收盘）。

- **采集源取错字段（根因一）**：`quote_sources` 腾讯解析原取第 3 位（**当前价**），
  注释误标为「今开」。仅当 09:26 前后 current==open 才恰好等于竞价价；一旦过连续
  竞价时段再采，就把盘中/收盘价当开盘价。实测校准字段位：3=当前价 **5=今开**
  4=昨收（东财 f46 本就正确，无需改）
- **采集时间窗无上界（根因二）**：`auction_scheduler_loop` 原 `now >= 09:26` 即触发、
  无上限。新增 `config.AUCTION_COLLECT_DEADLINE`（默认 09:30），采集仅在
  `[09:26, 09:30]` 内执行；超窗不再补采（**宁缺勿错**——竞价价不可回溯）
- **防重守卫不持久（根因三）**：`_auction_fired` 原为纯内存，进程重启即清空 → 晚间
  重启再次触发采集。改为内存 + 落盘双份（`sync_schedule.json` 新增 `auction_fired`
  字段，保留最近 14 天）；app.py 经 `schedule_fired_provider/marker` 注入
  （服务层不直连存储，沿用既有注入纪律）
- 抽出纯函数 `_auction_due(now_hm, guard)`（采集窗口 + 收口阈值 + 守卫），供调度与测试
- 测试：`test_quote_sources.test_open_is_field5_not_current_field3`（构造
  current≠open 的收盘后形态）+ `AuctionTriggerWindowTest` 6 用例（窗口边界/守卫
  持久化/合并/裁剪）；夹具修正腾讯行布局、隔离 `SCHEDULE_FILE`；Python 全量 409 全绿
- 影响面：**行情/仓库数据未受损**（K 线、分区经核验完整）；09-10 因当日 09:26 采集
  本已失败，误采未覆盖有效数据；打板指标用 `value_source=kline`，面板未被污染
- 部署动作：fnOS 本地构建 `stockdb-ai:0.10.35` → `up -d`；清理 09-10 误导告警
...
> fnOS 实测：引擎冷加载 22GB leveldb 需约 80~110s，60s 不够——0.10.31 部署后
> 重启仍触发熔断（21:17:38 三次探针失败 → 300s 暗灯 + 误报「行情数据不可用」
> + 调度追赶任务连接被拒）。

- `stockdb-ai/entrypoint.sh`：等待上限 60s → 300s（150 × 2s），就绪即 break——
  正常约 1~2 分钟起 webui；覆盖冷加载最坏情况与余量
- 验证：0.10.31 部署实测引擎就绪点约 80~110s、容器内 `curl 127.0.0.1:7899`
  返回 200、宿主机端口 0.33s；熔断 300s 后自愈（health 回 ok、data_latest
  20260910、latency 7ms）——证明熔断兜底有效，本次消除的是这段空窗
- Python 全量 398 全绿（entrypoint 无单测，改动为 shell 等待上限）
