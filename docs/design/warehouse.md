...
- **hk 迁仓的前置条件（0.10.43 补齐取证，按序清）**——文档此前只写了结论，未写前提：
  1. **口径必须归一到不复权**（阻塞项）：A 股 daily 存的是不复权原价 + 因子列
     （实测 20260911 全市场 `adj_factor`/`close_fq` 全 NULL 即"事件未接、原价入库"）；
     而 mydb `hk日k` 走的是东财 `fqt=1` / 腾讯 `qfq` = **前复权**。同一列两种口径，
     跨 A/H 对比即错。且**前复权每次除权后历史全量重算**，与 facts「只增不改」
     （分区文件存在即跳过）冲突——仓里会永久停在写入那天的口径且无从发现。
     故迁仓前必须先取 `fqt=0` / 去掉 `qfq` 的不复权价，并在写入契约里显式标注口径。
  2. **成交量/成交额单位对齐**：港股为「股」，A 股引擎给「手」——不换算即不可比。
  3. **交易日历**：已就绪——`core/calendar_market.py` 的 `HK` 实例（0.10.43），
     与 A 股同源（exchange_calendars）但**独立表**（2026 年差异：香港休/A股开 6 天、
     A股休/香港开 11 天），覆盖到 2027-09-13。
  4. **`prev_close` 需自行补**：港股源不返回昨收，而 pct_chg/涨停类判定依赖它。
  5. **codes 表需加 hk 行**（5 位码），否则 `v_codes` join 取不到港股。
  6. `is_st`/`pb`/`pe_ttm`/`float_mv` 港股源没有 → 长期 NULL，不编值。
- **台风/黑色暴雨临时休市不预判**（港股独有，交易所日历不收录、事后才知）：
  按"数据即事实"——拉不到就跳过该日，不预生成空行、不伪造闭市标记。
  代价是那天会成空洞（`watermark` 仍推进），由 `v_daily` 缺口巡检/补拉兜底。
